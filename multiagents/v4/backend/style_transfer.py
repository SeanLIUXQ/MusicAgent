...
from v3 import MusicAgent, load_docs_content, get_sonic_pi_naming_constraints, fix_reserved_word_variables
...
def style_transfer_sonic_pi(original_code, style_request, client, log_callback=None):
    ...
    style_transformer = MusicAgent(
        ...
        "CRITICAL: Never use Sonic Pi built-in function names (like chord, scale, play, sample, amp, pan, etc.) as variable names. Use descriptive alternatives like chord_notes, chord_progression, scale_pattern, my_amp, etc.",
        client
    )
    ...
    style_critic = MusicAgent(
        ...
        "CRITICAL: Check for variable naming conflicts with Sonic Pi built-in functions (chord, scale, play, sample, amp, pan, rate, etc.) and suggest alternatives if found.",
        client
    )
    ...
    style_arranger = MusicAgent(
        ...
        "CRITICAL: Ensure all variable names do not conflict with Sonic Pi built-in functions. Replace any reserved word variables with descriptive alternatives (e.g., 'chord' -> 'chord_notes' or 'chord_progression', 'scale' -> 'scale_pattern' or 'scale_notes').",
        client
    )
...
10. CRITICAL: Never use built-in function names (chord, scale, play, sample, amp, pan, rate, etc.) as variable names. Use alternatives like chord_notes, chord_progression, scale_pattern, my_amp, panning, etc."""}
    ]
...
4. Check if any variables use Sonic Pi built-in function names (chord, scale, play, sample, amp, pan, rate, etc.) - if found, suggest specific alternatives
5. What improvements can be made to better match the target style?
...
4. Replace ALL variables that conflict with Sonic Pi built-in functions with safe alternatives (e.g., 'chord' -> 'chord_notes' or 'chord_progression', 'scale' -> 'scale_pattern' or 'scale_notes')
5. Is wrapped with ```ruby ... ```"""}
    ]
    ...
    original_transformed = transformed_code
    transformed_code = fix_reserved_word_variables(transformed_code)
    if original_transformed != transformed_code:
        log("⚠️ Post-processing: Fixed reserved word variable names in transformed code\n")
