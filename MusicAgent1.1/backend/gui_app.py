...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QFrame, QScrollArea, QGroupBox, QComboBox, QFileDialog)
...
class ImportMusicThread(QThread):
    """Thread for importing music file (MIDI / audio) and converting to Sonic Pi code"""
    finished = pyqtSignal(str, str)  # Sonic Pi code, midi_path
    error = pyqtSignal(str)
    progress = pyqtSignal(str)
    log_message = pyqtSignal(str)

    def __init__(self, file_path, client, output_dir=None):
        super().__init__()
    ...
    def run(self):
        try:
            self.progress.emit("🎧 正在从音乐文件生成 Sonic Pi 代码...")
            import v3
            code, midi_path = v3.music_file_to_sonic_pi(
                self.file_path,
                self.client,
            ...
            if code:
                ...
                self.error.emit("从音乐文件生成代码失败（代理没有返回代码）")
        except Exception as e:
            self.error.emit(f"Error during music import: {str(e)}")
            import traceback
            self.log_message.emit(f"\n❌ Error in ImportMusicThread: {str(e)}\n")
            self.log_message.emit(f"Traceback:\n{traceback.format_exc()}\n")
...
class AudioImportThread(QThread):
    """Thread for importing audio file and converting to Sonic Pi code via Qwen-Omni."""
    finished = pyqtSignal(str, str, str)  # music_prompt, code, midi_path
    error = pyqtSignal(str)
    progress = pyqtSignal(str)
    log_message = pyqtSignal(str)

    def __init__(self, audio_path, client, midi_output_dir):
        super().__init__()
        self.audio_path = audio_path
        self.client = client
        self.midi_output_dir = Path(midi_output_dir)
    ...
    def run(self):
        try:
            self.progress.emit("🎧 正在使用 Qwen-Omni 分析音频...")
            self.log_message.emit(f"[AudioImport] Input file: {self.audio_path}")

            # 1) 使用 Qwen-Omni 从音频生成描述和 Sonic Pi 代码
            music_prompt, sonic_code = call_qwen_audio_to_code(self.audio_path)
            if not sonic_code or not sonic_code.strip():
                self.error.emit("Qwen-Omni 未返回有效的 Sonic Pi 代码")
                return

            # 2) 使用现有 compiler，将 Sonic Pi 代码编译为 MIDI（与文本流程保持一致）
            out_dir = self.midi_output_dir
            out_dir.mkdir(parents=True, exist_ok=True)
            midi_output = out_dir / f"audio_import_{int(time.time())}.mid"

            self.progress.emit("🔧 正在将 Sonic Pi 代码编译为 MIDI...")
            ...
            if not midi_path:
                self.log_message.emit("[AudioImport] MIDI 编译失败，将只返回 Sonic Pi 代码。")
                midi_path_str = ""
            else:
                midi_path_str = midi_path

            # 记录 Qwen 生成的描述，方便后续反馈使用
            self.log_message.emit(f"[AudioImport] Qwen 描述: {music_prompt}")
            self.finished.emit(music_prompt, sonic_code, midi_path_str)
        except Exception as e:
            import traceback
            self.error.emit(f"音频导入失败: {e}")
...
class MusicGeneratorGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 👉 新增：从音频文件导入按钮
        self.import_audio_btn = QPushButton("🎧 Import from Audio File")
        self.import_audio_btn.setObjectName("importAudioBtn")
        self.import_audio_btn.setFont(QFont("Segoe UI", 10))
        self.import_audio_btn.clicked.connect(self.on_import_audio_clicked)
        button_layout.addWidget(self.import_audio_btn)
        ...
        # 历史文件选择区域
        history_group = QGroupBox("📁 Historical generated files (for style transfer)")
        ...
        self.history_combo.setPlaceholderText("Select history file...")
        ...
        # 加载按钮
        self.load_history_btn = QPushButton("📂 Load selected file")
        ...
        refresh_btn.setToolTip(" Refresh the list of historical files")
    ...
    def init_client(self):
        ...
        # 优先使用环境变量，否则使用默认值
        import os
        api_key = os.getenv('DEEPSEEK_API_KEY','**')  # 请替换为您的 DeepSeek API Key
        ...
        if api_key == 'your_api_key_here':
            QMessageBox.warning(
                self,
                "API Key 未配置",
                "请设置 DEEPSEEK_API_KEY 环境变量或修改 gui_app.py 第 742 行的 API Key。\n\n"
                "获取 API Key:\n"
    ...
    def on_import_audio_clicked(self):
        """从音频文件导入，并通过 Qwen-Omni 生成 Sonic Pi 代码"""
        ...
        # 启动后台线程（避免卡住 GUI）
        self.audio_thread = AudioImportThread(
            audio_path=audio_path,
            client=self.client,
            midi_output_dir=self.midi_output_dir,
        )
        self.audio_thread.finished.connect(self.on_audio_import_finished)
        self.audio_thread.error.connect(self.on_audio_import_error)
        self.audio_thread.progress.connect(self.on_progress_update)
        self.audio_thread.log_message.connect(self.on_log_message)
    ...
    def on_generate_clicked(self):
        ...
        self.save_midi_btn.setEnabled(False)
        self.style_transfer_btn.setEnabled(False)
        self.import_music_btn.setEnabled(False)
    ...
    def on_progress_update(self, message):
        ...
        if "Error" in message or "Failed" in message or "❌" in message:
            ...
            """)
    def on_import_music_clicked(self):
        """从音乐文件（MIDI / 音频）生成 Sonic Pi 代码"""
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "选择音乐文件（MIDI / WAV / MP3）",
            "",
            "Music Files (*.mid *.midi *.wav *.mp3);;All Files (*)"
        )
        if not file_path:
            return
        ...
        self.status_label.setText("🎧 正在从音乐文件生成 Sonic Pi 代码...")
        self.status_label.setStyleSheet("""
            QLabel {
                color: #16a085;
                padding: 8px;
        ...
        # 启动导入线程
        self.import_thread = ImportMusicThread(
            file_path,
            self.client,
            output_dir=str(self.midi_output_dir)
        )
        ...
        self.import_thread.progress.connect(self.on_progress_update)
        self.import_thread.log_message.connect(self.on_log_message)
        self.import_thread.start()
    def on_import_finished(self, code, midi_path):
        """音乐文件导入完成后复用生成成功逻辑"""
        self.on_generation_finished(code, midi_path)
        # 确保导入按钮也重新启用
        self.import_music_btn.setEnabled(True)
    ...
    def on_audio_import_finished(self, music_prompt, code, midi_path):
        """音频导入完成后，将结果当作一次正常生成。"""
        # 让后续“反馈修改 / 风格转换”有一个合理的 original_prompt
        if music_prompt:
            self.original_prompt = music_prompt
        else:
            self.original_prompt = "(Imported from audio file)"

        # 复用原来的生成完成处理逻辑
        self.on_generation_finished(code, midi_path or "")

    def on_audio_import_error(self, error_msg: str):
        """处理音频导入错误：直接复用生成错误的 UI 逻辑"""
        self.on_generation_error(error_msg)
    ...
    def handle_audio_import_result(self, music_prompt: str, code: str, midi_path: str = ""):
        """
        当音频 → Sonic Pi 流程结束后调用。
        我们把它当成一次“正常的生成结果”，
        以便后续反馈 / 风格转换可以复用同一套逻辑。
        """
        # 把 Qwen 听完给的文字描述，当成 original_prompt 记录下来
        if music_prompt:
            self.original_prompt = music_prompt
        else:
            self.original_prompt = "(Imported from audio file)"

        # 复用原来的生成完成逻辑：这一步会更新 self.generated_code / self.midi_path 等
        self.on_generation_finished(code, midi_path or "")
