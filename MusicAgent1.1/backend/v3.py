...
# Pattern to remove "master_control" live loop that some LLMs append,
# e.g.:
...
#   stop
# end
MASTER_CONTROL_RE = re.compile(
    r"(?:#.*Optional:.*stop all loops gracefully[^\n]*\n)?\s*"
    r"live_loop\s*:master_control\s+do.*?end\s*",
    re.IGNORECASE | re.DOTALL,
)


def remove_master_control_loop(code: str) -> str:
    """
    Post-processing: remove the auto-added 'master_control' live loop block.

    Some model outputs append a helper like:
    ...
    This can interfere with normal playback, so we strip it from the final code.
    """
    if not code:
        return code

    new_code, count = MASTER_CONTROL_RE.subn("", code)
    # 保留结尾空行一致性
...
def multi_agent_generate_sonic_pi(prompt_text, client, user_feedback=None, previous_code=None, output_dir=None,
    ...
    if original_code2 != sonic_pi_code:
        ...
        # 最后一步后处理：移除自动添加的 master_control 终止循环
    original_code3 = sonic_pi_code
    sonic_pi_code = remove_master_control_loop(sonic_pi_code)
    if original_code3 != sonic_pi_code:
        log("⚠️ Post-processing: Removed auto-added 'master_control' live loop block\n")
